...
checkroot(){
    ...
    unset LD_PRELOAD
    if [ "$(id -u)" -ne 0 ]; then
        echo -e "${VERMELHO}Você deve ser usuário root para executar esse script!\n${BRANCO}"
        exit 1
    fi
...
apt_based(){
    ...
    if [ "$?" -ne 0 ]; then
        echo -e "${VERMELHO}Um erro ocorreu! Parece que o apt-get não funciona.\n${BRANCO}"
        exit 1
    fi
}
...
pacman_based(){
    ...
    pip install -r requirements.txt
    if [ "$?" -ne 0 ]; then
        echo -e "${VERMELHO}Um erro ocorreu! Parece que o pacman não funciona.\n${BRANCO}"
        exit 1
    fi
}
